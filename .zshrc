...
precmd() {
	vcs_info
	FIRST_PROMPT="%(!.%F{red}root%f.%F{green}$USER%f) %F{$prompt_color}%m%f %F{$(prompt_dir_writeable)}%~%f %* %F{$(prompt_git_dirty)}${vcs_info_msg_0_}%f %F{cyan}$(prompt_get_gcloud_config)%f $(prompt_get_gcloud_context) %F{blue}$(prompt_get_context) %F{cyan}$(prompt_get_namespace)%f %(1j.%j.)"
}
...
# load zsh-syntax-highlighting on Debian
